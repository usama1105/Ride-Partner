...
suspend inline fun <T : Any, reified V : Any> performPostRequest(
    apiName: String,
    model: T,
    encrypt: Boolean = false // 👈 added param
): Resource<V> {
    val response: Pair<Int, Resource<V>> = performPostRequestSimple(apiName, model, encrypt)
    return response.second
}
...
suspend inline fun <T : Any, reified V : Any> performPostRequestSimple(
    ...
    return try {
        ...
        if (response.isSuccessful) {
            ...
            return Pair(response.code(), Resource.Invalid(errorModel))
        }
                ...
                apiErrorModel.toErrorModel(
                    ...
                    description = when {
                        ex.message?.contains("unable to resolve host", ignoreCase = true) == true ||
                                ex.message?.contains("network is unreachable", ignoreCase = true) == true -> {
                            "Internet -> ${
                                ex.message.orEmpty().cleanExceptionMessage().ifEmpty {
                                    "Check your internet connection"
                                }
...
fun processRequest(input: String, encrypt: Boolean = false): Any {
    return if (encrypt) {
        RequestModel(input.encrypt()) // old payload behaviour
    } else {
        JsonParser.parseString(input).asJsonObject // new raw JSON behaviour
    }
}
...
fun String?.cleanExceptionMessage(): String {
    if (this.isNullOrEmpty()) return "Some error occurred."
    return this
        .replace(Regex("https?://[\\w.-]+(?:\\.[a-zA-Z]{2,})?(?:/[\\w.-]*)*/?"), "server")
        .replace(Regex("failed to connect to [\\w.-]+(?:\\.[a-zA-Z]{2,})?(?:/[\\w.-]*)*/?",
            RegexOption.IGNORE_CASE), "failed to connect to server")
        .replace(Regex("unable to resolve host [\\w.-]+(?:\\.[a-zA-Z]{2,})?",
            RegexOption.IGNORE_CASE), "failed to connect to server")
